...
- Android gameplay has no touch cast or touch hook affordance. Motion reads accelerometer, gravity, and gyro together once per update; it learns a two-cast profile before play, uses a back-then-opposite-snap sequence for casting, and permits a smaller profile-relative back hook only in `HOOK_WINDOW`. Desktop keyboard/test simulation must be explicitly guarded from Android. Keep raw sensor traces out of retained logs; emit only compact derived motion markers.
