...
func _test_injectable_motion() -> void:
	...
	var left_arm := _runtime_sweep(left_motion, Vector3(-2.4, 0, 0))
	var left_cast := _runtime_sweep(left_motion, Vector3(5.0, 0, 0))
	...
	var ramped_cock := _runtime_sweep(ramped_pixel_gesture, -pixel_axis * 1.32, Vector3(0, 0, 0.61), 2)
	...
	var ramped_snap := _runtime_sweep(ramped_pixel_gesture, pixel_axis * 1.60, Vector3(0, 0, 0.61), 2)
	...
	var one_cock_spike := _runtime_sweep(spike_motion, -pixel_axis * 1.60, Vector3(0, 0, 0.61), 1)
	var two_cock_spikes := _runtime_sweep(spike_motion, -pixel_axis * 1.60, Vector3(0, 0, 0.61), 1)
	var deliberate_cock := _runtime_sweep(spike_motion, -pixel_axis * 1.60, Vector3(0, 0, 0.61), 1)
	var one_snap_spike := _runtime_sweep(spike_motion, pixel_axis * 1.80, Vector3(0, 0, 0.61), 1)
	var two_snap_spikes := _runtime_sweep(spike_motion, pixel_axis * 1.80, Vector3(0, 0, 0.61), 1)
	var deliberate_snap := _runtime_sweep(spike_motion, pixel_axis * 1.80, Vector3(0, 0, 0.61), 1)
	...
	var cock_before_gap := _runtime_sweep(cock_gap_reset, -pixel_axis * 1.60, Vector3(0, 0, 0.61), 2)
	cock_gap_reset.queue_sample(_motion_sample(Vector3.ZERO, Vector3.ZERO)); cock_gap_reset.update(0.01, true, false)
	var cock_after_gap_two := _runtime_sweep(cock_gap_reset, -pixel_axis * 1.60, Vector3(0, 0, 0.61), 2)
	var cock_after_gap_three := _runtime_sweep(cock_gap_reset, -pixel_axis * 1.60, Vector3(0, 0, 0.61), 1)
	...
	_runtime_sweep(snap_gap_reset, -pixel_axis * 1.60, Vector3(0, 0, 0.61))
	var snap_before_gap := _runtime_sweep(snap_gap_reset, pixel_axis * 1.80, Vector3(0, 0, 0.61), 2, 0.03)
	snap_gap_reset.queue_sample(_motion_sample(Vector3.ZERO, Vector3.ZERO)); snap_gap_reset.update(0.01, true, false)
	var snap_after_gap_two := _runtime_sweep(snap_gap_reset, pixel_axis * 1.80, Vector3(0, 0, 0.61), 2, 0.03)
	var snap_after_gap_three := _runtime_sweep(snap_gap_reset, pixel_axis * 1.80, Vector3(0, 0, 0.61), 1, 0.03)
	...
	_runtime_sweep(low_impulse, -pixel_axis * 1.60, Vector3(0, 0, 0.61))
	...
	_runtime_sweep(cock_gyro_recovery, -pixel_axis * 1.32, Vector3(0, 0, 0.10))
	var recovered_cock_same_burst := _runtime_sweep(cock_gyro_recovery, -pixel_axis * 1.32, Vector3(0, 0, 0.61), 1)
	...
	_runtime_sweep(varied_pixel_snap, -pixel_axis * 1.32, Vector3(0, 0, 0.61))
	...
	expect(float(_runtime_sweep(gyro_recovery, pixel_axis * 1.60, Vector3(0, 0, 0.61), 1).cast_quality) > 0.0, "an above-threshold snap gyro miss can recover on a later valid frame after shape buildup")
	var below_axis_snap := MotionService.new(); below_axis_snap.set_profile(pixel_profile)
	_runtime_sweep(below_axis_snap, -pixel_axis * 1.32, Vector3(0, 0, 0.61))
	...
	_runtime_sweep(wrong_polarity_snap, Vector3(0.50, -0.866, 0.0) * 1.32, Vector3(0, 0, 0.61))
	...
	_runtime_sweep(fast_timing, -pixel_axis * 1.32, Vector3(0, 0, 0.61))
	var fast_timing_cast := _runtime_sweep(fast_timing, pixel_axis * 8.0, Vector3(0, 0, 0.61), 3, 0.06)
	var late_timing := MotionService.new(); late_timing.set_profile(pixel_profile)
	_runtime_sweep(late_timing, -pixel_axis * 1.32, Vector3(0, 0, 0.61))
	var late_timing_cast := _runtime_sweep(late_timing, pixel_axis * 8.0, Vector3(0, 0, 0.61), 3, 0.18)
	var expired_timing := MotionService.new(); expired_timing.set_profile(pixel_profile)
	_runtime_sweep(expired_timing, -pixel_axis * 1.32, Vector3(0, 0, 0.61))
	var expired_timing_cast := _runtime_sweep(expired_timing, pixel_axis * 8.0, Vector3(0, 0, 0.61), 3, 0.22)
	...
	quiet_cock_recovery.update(0.05, true, false)
	var recovered_cock := _runtime_sweep(quiet_cock_recovery, Vector3(2.5, 0, 0), Vector3(0, 0, 0.5))
	var recovered_cast := _runtime_sweep(quiet_cock_recovery, Vector3(-3.8, 0, 0), Vector3(0, 0, 0.5))
	...
	var diagonal_arm := _runtime_sweep(diagonal_motion, diagonal_cock)
	var diagonal_cast := _runtime_sweep(diagonal_motion, -diagonal_cock * 1.70)
	...
	var residual_arm := _runtime_sweep(residual_cock_motion, Vector3(2.8, 0, 0), Vector3(0, 0, 0.55))
	...
	var residual_snap := _runtime_sweep(residual_cock_motion, Vector3(-4.4, 0, 0), Vector3(0, 0, 0.55))
	...
	var recovered_arm := _runtime_sweep(opposite_recovery, Vector3(2.8, 0, 0), Vector3(0, 0, 0.5))
	var recovered_snap := _runtime_sweep(opposite_recovery, Vector3(-4.4, 0, 0), Vector3(0, 0, 0.5))
	...
	_runtime_sweep(diagonal_right_snap, diagonal_cock)
	...
	_runtime_sweep(diagonal_off_axis, diagonal_cock)
	...
	var slow_back := _runtime_sweep(reversed_motion, Vector3(2.2, 0, 0))
	...
	_runtime_sweep(slow_motion, Vector3(2.2, 0, 0))
	var slow_snap := _runtime_sweep(slow_motion, Vector3(-4.8, 0, 0), Vector3(0, 0, 0.72), 3, 0.08)
	expect(float(slow_snap.cast_quality) > 0.0, "right cock then left snap arms and casts")
	var fast_motion := _calibrated_motion()
	_runtime_sweep(fast_motion, Vector3(2.3, 0, 0))
	var fast_snap := _runtime_sweep(fast_motion, Vector3(-4.8, 0, 0), Vector3(0, 0, 0.72), 3, 0.06)
	...
	_runtime_sweep(too_slow_motion, Vector3(2.4, 0, 0))
	var too_slow := _runtime_sweep(too_slow_motion, Vector3(-5.0, 0, 0), Vector3(0, 0, 0.72), 3, 0.22)
	...
	var easier_back := _runtime_sweep(sensitivity_motion, Vector3(1.3, 0, 0))
