...
func _test_injectable_motion() -> void:
	...
	ramped_pixel_gesture.queue_sample(_motion_sample(pixel_axis * 1.60, Vector3(0, 0, 0.61)))
	var ramped_snap := ramped_pixel_gesture.update(0.12, true, false)
	expect(not weak_cock.cast_arm and ramped_cock.cast_arm and float(weak_snap.cast_quality) == 0.0 and float(ramped_snap.cast_quality) > 0.0, "a v16-level weak snap stays inert, then a >1.55/.61 ramp frame casts without a quiet gap")
	...
	var varied_pixel_rejected := varied_pixel_snap.update(0.08, true, false)
	varied_pixel_snap.queue_sample(_motion_sample(pixel_axis * 1.60, Vector3(0, 0, 0.61)))
	var varied_pixel_recovered := varied_pixel_snap.update(0.10, true, false)
	...
	axis_recovery.queue_sample(_motion_sample(pixel_axis * 1.60, Vector3(0, 0, 0.61)))
	...
	gyro_recovery.queue_sample(_motion_sample(pixel_axis * 1.60, Vector3(0, 0, 0.10))); gyro_recovery.update(0.06, true, false)
	gyro_recovery.queue_sample(_motion_sample(pixel_axis * 1.60, Vector3(0, 0, 0.61)))
	...
	expect(float(below_axis_snap.update(0.18, true, false).cast_quality) == 0.0, "a snap below the restored .62 learned-axis gate remains rejected")
	...
	fast_timing.queue_sample(_motion_sample(pixel_axis * 8.0, Vector3(0, 0, 0.61)))
	...
	late_timing.queue_sample(_motion_sample(pixel_axis * 8.0, Vector3(0, 0, 0.61)))
	...
	expired_timing.queue_sample(_motion_sample(pixel_axis * 8.0, Vector3(0, 0, 0.61)))
...
func _test_project_source_settings() -> void:
	...
	expect(int(export_config.get_value("preset.0.options", "version/code")) == 17 and export_config.get_value("preset.0.options", "version/name") == "0.3.4-gaterestore1", "gate-restoration hotfix package version is bumped")
	...
	expect("PROFILE_HANDEDNESS_ALIGNMENT" in motion_source and "CALIBRATION_HANDEDNESS_ALIGNMENT" in motion_source and "RUNTIME_X_POLARITY_ALIGNMENT" in motion_source and "_snap_axis_tolerance" in motion_source and "_snap_polarity_tolerance" in motion_source and "_snap_gyro_threshold" in motion_source and "RUNTIME_FULL_REVERSAL_SECONDS" in motion_source and "RUNTIME_MAX_REVERSAL_SECONDS" in motion_source and "_burst_failure_latched" in motion_source and "MOTION_FAIL stage=%s reason=%s count=%d" in motion_source and not "SNAP_MIN_AXIS_TOLERANCE" in motion_source and not "MOTION_FAIL linear=" in motion_source and not "MOTION_FAIL gyro=" in motion_source and "sqrt(raw_fight_load)" in motion_source, "continuous burst recovery retains restored profile/gyro gates and stage-labeled derived-only diagnostics")
