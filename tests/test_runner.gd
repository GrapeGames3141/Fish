...
func _test_project_source_settings() -> void:
	...
	expect(int(export_config.get_value("preset.0.options", "version/code")) == 10 and export_config.get_value("preset.0.options", "version/name") == "0.2.1-ui2", "UI correction package version is bumped")
	...
	expect(not "HOLD TO CAST" in main_source and not "SET HOOK" in main_source and not "SAFE BYPASS" in main_source and not "cast_rect" in main_source and not "reel_center" in main_source and not "InputEventScreenDrag" in main_source and not "_handle_drag" in main_source and not "reel_fallback" in main_source and "not OS.has_feature(\"android\")" in main_source and "MOTION_FIGHT caught elapsed=" in main_source and "MOTION_FIGHT escaped elapsed=" in main_source and "_record_catch_once" in main_source and "records-screen-v01.png" in main_source and "ROD_TIP_ANCHORS" in main_source and "_rod_tip_for_frame" in main_source and not "draw_line(Vector2(87, 1070)" in main_source, "Android UI retains no touch gameplay paths and maps each rod frame's external line from terminal-guide anchors instead of the old handle anchor")
