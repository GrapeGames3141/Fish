...
func _test_project_source_settings() -> void:
	...
	expect(int(export_config.get_value("preset.0.options", "version/code")) == 27 and export_config.get_value("preset.0.options", "version/name") == "0.4.1-navfix1", "navigation correction package version is bumped")
	...
	expect("build/**" in excluded and "reports/**" in excluded and "art/ui_v1/mockups/**" in excluded and "art/ui_v1/runtime_source/catch-frame-clean-v01.png" in excluded and "art/ui_v1/runtime_source/bluegill-v01.png" in excluded and "art/ui_v1/runtime_source/app-icon-v01.png" in excluded and "art/ui_v1/runtime_source/records-screen-v01.png" in excluded and "art/ui_v1/runtime_source/rod-bend-strip-v01.png" in excluded and "addons/admob/internal/editor/**" in excluded and "addons/admob/internal/mock/**" in excluded and not "addons/admob/gdscript/src/mediation/**" in excluded, "mockups and superseded unreferenced masters are excluded while runtime mediation dependencies remain")
	...
	expect("MOTION_SETTLE_SECONDS := 0.30" in haptic_source and "is_motion_guarded" in haptic_source and "_motion_guard_remaining" in haptic_source, "haptic pulses own the v25 deterministic sensor-settle guard")
	...
	expect("randf()" in main_source and not "haptics.cue(\"cock\")" in main_source and "haptics.cue(\"bite\")" in main_source and "haptics.cue(\"hook\")" in main_source and "haptics.set_enabled(bool(save.data.settings[key]))" in main_source and "MOTION_CAST_CANCEL reason=timeout" in main_source and "session.state == FishingSession.State.CAST_ARMED or (session.state == FishingSession.State.READY and not haptics.is_motion_guarded())" in main_source and "session.state == FishingSession.State.HOOK_WINDOW and not haptics.is_motion_guarded()" in main_source and "session.state == FishingSession.State.REELING" in main_source and "snap_projection=%.2f" in main_source and "cock_projection=%.2f" in main_source and "MOTION_HOOK state=REELING projection=%.2f alignment=%.2f gyro=%.2f sweep_samples=%d" in main_source, "physical arm selects a fresh weighted fish without motor feedback; guarded hooks log only derived sweep scalars while CAST_ARMED and REELING remain active")
	...
	expect(not "HOLD TO CAST" in main_source and not "SET HOOK" in main_source and not "SAFE BYPASS" in main_source and not "cast_rect" in main_source and not "reel_center" in main_source and not "InputEventScreenDrag" in main_source and not "_handle_drag" in main_source and not "reel_fallback" in main_source and "COCK RIGHT, THEN SNAP LEFT" in main_source and "FISH ON — WAIT FOR THE PULSES" in main_source and "BITE — PULL RIGHT" in main_source and "session.state == FishingSession.State.HOOK_WINDOW" in main_source and "TILT LEFT TO EASE" in main_source and "TILT RIGHT TO PULL" in main_source and "not OS.has_feature(\"android\")" in main_source and "MOTION_FIGHT caught elapsed=" in main_source and "MOTION_FIGHT escaped elapsed=" in main_source and "_record_catch_once" in main_source and "records-screen-v02.png" in main_source and not "records-screen-v01.png\")" in main_source and "ROD_TIP_ANCHORS" in main_source and "_rod_tip_for_frame" in main_source and not "draw_line(Vector2(87, 1070)" in main_source and "RECORD 10 CASTS" in main_source and "cast_capture.is_active()" in main_source and "_start_cast_capture" in main_source and not "DIAGNOSTIC_AUTO_CAPTURE_ON_ANDROID" in main_source and not "_start_diagnostic_capture_once" in main_source and not "diagnostic_capture_autostart_started" in main_source and "DisplayServer.get_display_safe_area" in main_source and "_virtual_safe_top" in main_source and "set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)" in main_source and "mouse_filter = Control.MOUSE_FILTER_STOP" in main_source and "font = ThemeDB.fallback_font" in main_source and "queue_redraw()" in main_source, "bite haptics are guarded before hook detection, the full-viewport view preserves safe-top chrome/touch handling, and gameplay has no touch path")
	...
	expect(not "catch-frame-clean-v01.png" in main_source and not "catch_frame_texture" in main_source and "NEW PB" in main_source and "TAP TO CONTINUE" in main_source, "catch reveal uses a scenic native plaque rather than a trophy frame")
	var records_source := main_source.get_slice("func _draw_records() -> void:", 1).get_slice("func _fish_definition", 0)
	expect("records-screen-v02.png" in main_source and "bluegill\", \"largemouth_bass\", \"channel_catfish\", \"rainbow_trout\", \"smallmouth_bass\", \"northern_pike" in records_source and "best > 0.0 else \"—\"" in records_source and "name_centers" in records_source and "stat_centers" in records_source and "_centered_text" in records_source and not "_draw_fish_atlas_contained" in records_source and not "draw_style_box(_panel_style" in records_source and not "_text(\"CATCH RECORDS\"" in records_source and not "draw_style_box(_control_style(\"plaque_normal\")" in records_source, "Records uses the cohesive v02 master and overlays only row-major native names/stats without duplicate runtime framing, fish, or cards")
	...
	expect("control-kit-alpha-v01.png" in main_source and "StyleBoxTexture.new()" in main_source and "_control_style" in main_source and "plaque_normal" in main_source and "plaque_pressed" in main_source and "plaque_disabled" in main_source and "row_selected" in main_source and "texture_margin_left = 22.0" in main_source and "texture_margin_top = 22.0" in main_source and "texture_margin_right = 22.0" in main_source and "texture_margin_bottom = 22.0" in main_source and "draw_center = true" in main_source and not "set_patch_margin" in main_source and "TAP SETTINGS TO CLOSE" in main_source and not "Tap title to close" in main_source and "BACK TO FISHING" in main_source and "SETTINGS" in main_source and "SELECTED" in main_source and not "draw_circle(Vector2(580" in main_source and not "_text(\"⚙\"" in main_source, "control-kit provides Godot 4.7 texture-margin nine-slice skins while Settings and exit plaques retain native labels")
	expect("top-nav-strip-v01.png" in main_source and "_refresh_top_nav_geometry()" in main_source and "_handle_press(pos: Vector2)" in main_source and "records_rect" in main_source and "locations_rect" in main_source and "settings_rect" in main_source and "RECORDS" in main_source and "WATERS" in main_source and "SETTINGS" in main_source and "records_available" in main_source and "LOCKED" in main_source and "if settings_rect.has_point(pos): overlay = \"settings\"" in main_source and "controller._can_open_records()" in main_source and not "_can_open_journal" in main_source and not "menu_medallion" in main_source, "top navigation shares current safe-area geometry for drawing and hits; Settings remains available while Records and Waters visibly gate active fishing")
	var locations_source := main_source.get_slice("func _draw_locations() -> void:", 1).get_slice("func _draw_location_card", 0)
	expect("records_safe_top" in main_source and "safe_top_override = 91.0" in main_source and "_draw_records_back_to_fishing(safe_top)" in main_source and "_draw_back_to_fishing(safe_top)" in locations_source and "_refresh_records_back_to_fishing_rect" in main_source and "BACK TO FISHING" in main_source and not "back_medallion" in records_source and not "back_medallion" in locations_source and not "_text(\"BACK\"" in records_source and not "_text(\"BACK\"" in locations_source, "Records keeps its authored crest clear while Records and Locations use safe-aware Back to Fishing plaques without an arrow/page-turn metaphor")
	...
	expect("settings_footer_close_rect := Rect2(110, 1010, 500, 90)" in main_source and "draw_style_box(_control_style(\"plaque_normal\"), settings_footer_close_rect)" in main_source and "settings_footer_close_rect.has_point(pos) or pos.y < 270" in main_source, "the visible Settings footer plaque is an explicit deterministic close target")
