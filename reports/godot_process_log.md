...
| Android export | Console `1124`; worker `32360` | Exit `0`; emitted `build/android/CastAndCrank-Gate1-debug.apk`, 92,213,872 bytes, SHA-256 `E2109E142F91F5C1204C9C9371C51F7AC690C4427B78C150CCFCBA4AD87C563F`. Package inspection confirmed `com.tak.castandcrank`, min SDK 24, target SDK 36. `apksigner` rejected it as unsigned (missing `META-INF/MANIFEST.MF`), so it is not a distributable debug artifact. Signed retry pending. |
