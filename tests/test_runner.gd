...
func _test_project_source_settings() -> void:
	...
	expect(int(export_config.get_value("preset.0.options", "version/code")) == 26 and export_config.get_value("preset.0.options", "version/name") == "0.4.0-ui3", "UI v2 package version is bumped")
	...
	expect("control-kit-alpha-v01.png" in main_source and "StyleBoxTexture.new()" in main_source and "_control_style" in main_source and "_draw_control_region" in main_source and "plaque_normal" in main_source and "plaque_pressed" in main_source and "plaque_disabled" in main_source and "row_selected" in main_source and "menu_medallion" in main_source and "settings_medallion" in main_source and "back_medallion" in main_source and "texture_margin_left = 22.0" in main_source and "texture_margin_top = 22.0" in main_source and "texture_margin_right = 22.0" in main_source and "texture_margin_bottom = 22.0" in main_source and "draw_center = true" in main_source and not "set_patch_margin" in main_source and "TAP SETTINGS TO CLOSE" in main_source and not "Tap title to close" in main_source and "records_back_rect" in main_source and "SETTINGS" in main_source and "SELECTED" in main_source and not "draw_circle(Vector2(580" in main_source and not "_text(\"⚙\"" in main_source, "control-kit provides Godot 4.7 texture-margin nine-slice skins while Records has one safe-aware back control and Settings labels its bottom close plaque")
	expect("records_safe_top" in main_source and "safe_top_override = 91.0" in main_source and "records_back_rect = Rect2(36, safe_top + 18, 64, 64)" in main_source and "_draw_control_region(\"back_medallion\", Rect2(41, safe_top + 23, 54, 54))" in main_source and not "draw_rect(Rect2(0, 0, 154, safe_top + 92)" in main_source and not "content_shift := maxf(0.0, safe_top + 24.0 - 58.0)" in main_source and "records_back_rect.has_point(pos)" in main_source, "Records has one safe-aware 64px back affordance without masking or moving the cohesive authored plate")
