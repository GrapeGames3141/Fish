...
func _test_injectable_motion() -> void:
	...
	expect(is_equal_approx(thresholds._back_threshold(), 1.16) and is_equal_approx(thresholds._forward_threshold(), 0.72) and is_equal_approx(thresholds._hook_threshold(), 0.8) and is_equal_approx(thresholds._gyro_threshold(), 0.32) and is_equal_approx(thresholds._snap_gyro_threshold(), 0.18), "gesture-fix thresholds apply sensitivity to each complete exact max formula")
	thresholds.sensitivity = 0.5; expect(is_equal_approx(thresholds._gyro_threshold(), 0.60) and is_equal_approx(thresholds._snap_gyro_threshold(), 0.36), "cast and snap gyro thresholds retain their distinct caps")
	...
	expect(absf(pixel_thresholds._back_threshold() - 1.244506) <= 0.001 and absf(pixel_thresholds._forward_threshold() - 0.883536) <= 0.001 and is_equal_approx(pixel_thresholds._snap_axis_tolerance(), 0.4216) and is_equal_approx(pixel_thresholds._snap_polarity_tolerance(), 0.081) and is_equal_approx(pixel_thresholds._gyro_threshold(), 0.60) and is_equal_approx(pixel_thresholds._snap_gyro_threshold(), 0.36) and is_equal_approx(pixel_thresholds._gyro_threshold() * 0.65, 0.39), "Pixel gesture-fix gates set cock 1.2445, snap .8835, cast gyro .60, snap gyro .36, and unchanged hook gyro .39")
	...
	ramped_pixel_gesture.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61)))
	var ramped_cock := ramped_pixel_gesture.update(0.04, true, false)
	ramped_pixel_gesture.queue_sample(_motion_sample(pixel_axis * 0.70, Vector3(0, 0, 0.61)))
	var weak_snap := ramped_pixel_gesture.update(0.05, true, false)
	ramped_pixel_gesture.queue_sample(_motion_sample(pixel_axis * 1.05, Vector3(0, 0, 0.61)))
	var ramped_snap := ramped_pixel_gesture.update(0.12, true, false)
	expect(not weak_cock.cast_arm and ramped_cock.cast_arm and float(weak_snap.cast_quality) == 0.0 and float(ramped_snap.cast_quality) > 0.0, "sub-threshold cock/snap ramp frames stay inert until they cross their v16 gates")
	...
	cock_gyro_recovery.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.10)))
	cock_gyro_recovery.update(0.05, true, false)
	cock_gyro_recovery.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61)))
	var recovered_cock_same_burst := cock_gyro_recovery.update(0.05, true, false)
	...
	varied_pixel_snap.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61)))
	varied_pixel_snap.update(0.04, true, false)
	varied_pixel_snap.queue_sample(_motion_sample(varied_left_direction * 3.0, Vector3(0, 0, 0.61)))
	var varied_pixel_cast := varied_pixel_snap.update(0.18, true, false)
	expect(varied_match >= 0.4216 and varied_match < 0.62 and varied_polarity >= 0.081 and float(varied_pixel_cast.cast_quality) > 0.0, "a natural varied-angle left snap around .45 learned-axis match now casts")
	...
	axis_recovery.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61))); axis_recovery.update(0.01, true, false)
	axis_recovery.queue_sample(_motion_sample(below_axis_direction * 3.0, Vector3(0, 0, 0.61))); axis_recovery.update(0.06, true, false)
	axis_recovery.queue_sample(_motion_sample(pixel_axis * 1.2, Vector3(0, 0, 0.61)))
	expect(float(axis_recovery.update(0.10, true, false).cast_quality) > 0.0, "an above-threshold snap axis miss can recover on a later valid frame in the same burst")
	var gyro_recovery := MotionService.new(); gyro_recovery.set_profile(pixel_profile)
	gyro_recovery.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61))); gyro_recovery.update(0.01, true, false)
	gyro_recovery.queue_sample(_motion_sample(pixel_axis * 1.2, Vector3(0, 0, 0.10))); gyro_recovery.update(0.06, true, false)
	gyro_recovery.queue_sample(_motion_sample(pixel_axis * 1.2, Vector3(0, 0, 0.61)))
	expect(float(gyro_recovery.update(0.10, true, false).cast_quality) > 0.0, "an above-threshold snap gyro miss can recover on a later valid frame in the same burst")
	var below_axis_snap := MotionService.new(); below_axis_snap.set_profile(pixel_profile)
	below_axis_snap.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61)))
	below_axis_snap.update(0.04, true, false)
	below_axis_snap.queue_sample(_motion_sample(below_axis_direction * 3.0, Vector3(0, 0, 0.61)))
	expect(float(below_axis_snap.update(0.18, true, false).cast_quality) == 0.0, "a snap below the .38 learned-axis floor remains rejected")
	...
	wrong_polarity_snap.queue_sample(_motion_sample(Vector3(0.50, -0.866, 0.0) * 1.32, Vector3(0, 0, 0.61)))
	wrong_polarity_snap.update(0.04, true, false)
	wrong_polarity_snap.queue_sample(_motion_sample(Vector3(0.05, 0.9987, 0.0) * 3.0, Vector3(0, 0, 0.61)))
	expect(float(wrong_polarity_snap.update(0.18, true, false).cast_quality) == 0.0, "a learned-axis-compatible snap with the wrong physical left polarity remains rejected")
	var fast_timing := MotionService.new(); fast_timing.set_profile(pixel_profile)
	fast_timing.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61))); fast_timing.update(0.01, true, false)
	fast_timing.queue_sample(_motion_sample(pixel_axis * 4.0, Vector3(0, 0, 0.61)))
	var fast_timing_cast := fast_timing.update(0.18, true, false)
	var late_timing := MotionService.new(); late_timing.set_profile(pixel_profile)
	late_timing.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61))); late_timing.update(0.01, true, false)
	late_timing.queue_sample(_motion_sample(pixel_axis * 4.0, Vector3(0, 0, 0.61)))
	var late_timing_cast := late_timing.update(0.54, true, false)
	var expired_timing := MotionService.new(); expired_timing.set_profile(pixel_profile)
	expired_timing.queue_sample(_motion_sample(-pixel_axis * 1.32, Vector3(0, 0, 0.61))); expired_timing.update(0.01, true, false)
	expired_timing.queue_sample(_motion_sample(pixel_axis * 4.0, Vector3(0, 0, 0.61)))
	var expired_timing_cast := expired_timing.update(0.66, true, false)
	...
	var diagnostic_motion := _calibrated_motion(); diagnostic_motion.queue_sample(_motion_sample(Vector3(2.0, 4.0, 0))); diagnostic_motion.update(0.05, true, false)
	...
	for frame in range(4): burst_motion.queue_sample(_motion_sample(Vector3(2.0, 4.0, 0))); burst_motion.update(0.05, true, false)
	...
	quiet_cock_recovery.queue_sample(_motion_sample(Vector3(2.0, 4.0, 0), Vector3(0, 0, 0.5)))
	quiet_cock_recovery.update(0.05, true, false)
	quiet_cock_recovery.queue_sample(_motion_sample(Vector3.ZERO, Vector3.ZERO))
	quiet_cock_recovery.update(0.05, true, false)
	quiet_cock_recovery.queue_sample(_motion_sample(Vector3(2.5, 0, 0), Vector3(0, 0, 0.5)))
	var recovered_cock := quiet_cock_recovery.update(0.05, true, false)
	quiet_cock_recovery.queue_sample(_motion_sample(Vector3(-3.8, 0, 0), Vector3(0, 0, 0.5)))
	var recovered_cast := quiet_cock_recovery.update(0.18, true, false)
	...
	diagonal_off_axis.queue_sample(_motion_sample(Vector3(-0.10, 0.0, -1.0).normalized() * 12.0))
...
func _test_project_source_settings() -> void:
	...
	expect(int(export_config.get_value("preset.0.options", "version/code")) == 16 and export_config.get_value("preset.0.options", "version/name") == "0.3.3-gesturefix1", "continuous-gesture hotfix package version is bumped")
	...
	expect("PROFILE_HANDEDNESS_ALIGNMENT" in motion_source and "CALIBRATION_HANDEDNESS_ALIGNMENT" in motion_source and "RUNTIME_X_POLARITY_ALIGNMENT" in motion_source and "SNAP_MIN_AXIS_TOLERANCE" in motion_source and "_snap_axis_tolerance" in motion_source and "_snap_polarity_tolerance" in motion_source and "_snap_gyro_threshold" in motion_source and "RUNTIME_FULL_REVERSAL_SECONDS" in motion_source and "RUNTIME_MAX_REVERSAL_SECONDS" in motion_source and "_burst_failure_latched" in motion_source and "MOTION_FAIL stage=%s reason=%s count=%d" in motion_source and not "MOTION_FAIL linear=" in motion_source and not "MOTION_FAIL gyro=" in motion_source and "sqrt(raw_fight_load)" in motion_source, "continuous gesture recognition retains distinct snap gates and stage-labeled derived-only diagnostics")
