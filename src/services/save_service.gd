...
var path: String
func _init(custom_path: String = PATH) -> void: path = custom_path
static func default_data() -> Dictionary:
	var catches := {}; var best_cm := {}
	for fish_id in PLANNED_FISH_IDS: catches[fish_id] = 0; best_cm[fish_id] = 0.0
	return {"version": VERSION, "calibrated": false, "motion_profile": {}, "selected_location_id": "pine_lake", "settings": {"sensitivity": 1.0, "haptics": true, "audio": true, "reduced_motion": false, "left_handed": false}, "catches": catches, "best_cm": best_cm}
...
func record_catch(fish_id: String, length_cm: float) -> void: data.catches[fish_id] = int(data.catches.get(fish_id, 0)) + 1; data.best_cm[fish_id] = maxf(float(data.best_cm.get(fish_id, 0.0)), length_cm); save_data()
