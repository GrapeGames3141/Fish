...
- Android exports must set `permissions/vibrate=true`. Default Android haptic dispatch uses `AndroidRuntime` application context and vibrator service: API 26+ uses `VibrationEffect.createOneShot` with amplitude, while supported min-SDK API 24–25 uses legacy duration-only `vibrate`. Retain injected test emission and `Input.vibrate_handheld` fallback when unavailable. Test export settings before a replacement device install.
