...
const frameDir = 'E:/CodexCache/haptic-fish-water-preview-v02/frames';
...
const gifOutput = path.join(gallery, 'willow-water-motion-v02.gif');
const mp4Output = path.join(gallery, 'willow-water-motion-v02.mp4');
...
async function main() {
  ...
  fs.writeFileSync(path.join(gallery,'willow-water-motion-v02-mask.png'), Buffer.from(maskDataUrl.split(',')[1], 'base64'));
  const frameIndexes = keyframesOnly ? [0, 20, 40, 60, 80] : Array.from({ length: FRAMES }, (_, i) => i);
  console.log(`water preview: rendering ${frameIndexes.length} ${keyframesOnly ? 'keyframes' : 'frames'}`);
  ...
  if (keyframesOnly) {
    for (const frame of frameIndexes) fs.copyFileSync(path.join(frameDir, `frame-${String(frame).padStart(3,'0')}.png`), path.join(gallery, `willow-water-motion-v02-frame-${String(frame).padStart(3,'0')}.png`));
    console.log('water preview: keyframes ready');
    return;
  }
  console.log('water preview: recording MP4');
  // The player-friendly full-colour companion is recorded from the same canvas.
  const mp4 = await page.evaluate(async ({ seconds, fps }) => {
    const stream = document.querySelector('#view').captureStream(fps);
    const chunks=[]; const rec=new MediaRecorder(stream, { mimeType:'video/mp4;codecs=avc1.42E01E', videoBitsPerSecond: 7_000_000 });
    ...
    rec.stop(); await done; const blob=new Blob(chunks,{type:'video/mp4'}); const data=await blob.arrayBuffer(); return Array.from(new Uint8Array(data));
  }, { seconds: SECONDS, fps: FPS });
  fs.writeFileSync(mp4Output, Buffer.from(mp4));
  } finally {
  ...
  const checks = {
    ...
    gif: { path: gifOutput, bytes: fs.statSync(gifOutput).size }, mp4: { path: mp4Output, bytes: fs.statSync(mp4Output).size },
    mask: path.join(gallery,'willow-water-motion-v02-mask.png'),
  ...
  fs.copyFileSync(path.join(frameDir,'frame-000.png'), path.join(gallery,'willow-water-motion-v02-frame-000.png'));
  fs.copyFileSync(path.join(frameDir,'frame-040.png'), path.join(gallery,'willow-water-motion-v02-frame-040.png'));
  fs.copyFileSync(path.join(frameDir,'frame-080.png'), path.join(gallery,'willow-water-motion-v02-frame-080.png'));
