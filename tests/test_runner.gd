...
func _test_injectable_motion() -> void:
	...
	expect(is_equal_approx(thresholds._back_threshold(), 0.84) and is_equal_approx(thresholds._forward_threshold(), 0.96) and is_equal_approx(thresholds._hook_threshold(), 0.8) and is_equal_approx(thresholds._gyro_threshold(), 0.32), "snap-fix thresholds apply sensitivity to each complete exact max formula")
	...
	expect(absf(pixel_thresholds._back_threshold() - 1.02992) <= 0.001 and absf(pixel_thresholds._forward_threshold() - 1.178048) <= 0.001 and is_equal_approx(pixel_thresholds._snap_axis_tolerance(), 0.4216) and is_equal_approx(pixel_thresholds._snap_polarity_tolerance(), 0.081) and is_equal_approx(pixel_thresholds._gyro_threshold(), 0.60) and is_equal_approx(pixel_thresholds._gyro_threshold() * 0.65, 0.39), "Pixel snap-fix gates retune to cock 1.030, snap 1.178, axis .4216, polarity .081, gyro .60, hook gyro .39")
	var pixel_axis := Vector3(-0.8526, 0.2737, -0.4452).normalized()
	var weak_pixel_snap := MotionService.new(); weak_pixel_snap.set_profile(pixel_profile)
	weak_pixel_snap.queue_sample(_motion_sample(-pixel_axis * 1.20, Vector3(0, 0, 0.61)))
	var weak_pixel_arm := weak_pixel_snap.update(0.04, true, false)
	weak_pixel_snap.queue_sample(_motion_sample(pixel_axis * 1.25, Vector3(0, 0, 0.61)))
	var weak_pixel_cast := weak_pixel_snap.update(0.18, true, false)
	expect(weak_pixel_arm.cast_arm and float(weak_pixel_cast.cast_quality) > 0.0, "a snap just above the new Pixel 1.178 gate now casts although it was below v14's 1.546 gate")
	...
	varied_pixel_snap.queue_sample(_motion_sample(-pixel_axis * 1.20, Vector3(0, 0, 0.61)))
	...
	below_axis_snap.queue_sample(_motion_sample(-pixel_axis * 1.20, Vector3(0, 0, 0.61)))
	below_axis_snap.update(0.04, true, false)
	below_axis_snap.queue_sample(_motion_sample(Vector3(-0.10, 0.995, 0.0).normalized() * 4.0, Vector3(0, 0, 0.61)))
	...
	wrong_polarity_snap.queue_sample(_motion_sample(Vector3(0.50, -0.866, 0.0) * 1.20, Vector3(0, 0, 0.61)))
...
func _test_project_source_settings() -> void:
	...
	expect(int(export_config.get_value("preset.0.options", "version/code")) == 15 and export_config.get_value("preset.0.options", "version/name") == "0.3.2-snapfix1", "snap-left hotfix package version is bumped")
	...
	expect("PROFILE_HANDEDNESS_ALIGNMENT" in motion_source and "CALIBRATION_HANDEDNESS_ALIGNMENT" in motion_source and "RUNTIME_X_POLARITY_ALIGNMENT" in motion_source and "SNAP_MIN_AXIS_TOLERANCE" in motion_source and "_snap_axis_tolerance" in motion_source and "_snap_polarity_tolerance" in motion_source and "LOAD_RESPONSE_SECONDS" in motion_source and "MOTION_FAIL stage=%s reason=%s count=%d" in motion_source and not "MOTION_FAIL linear=" in motion_source and not "MOTION_FAIL gyro=" in motion_source and "sqrt(raw_fight_load)" in motion_source, "mirrored learned-axis motion retains snap-specific tolerance and stage-labeled derived-only diagnostics")
